...
<!-- TODO: Update figure with Windows GUI -->
...
<!-- NOTE: BiocManager::install() actually automatically installs packages from CRAN, Bioc and GitHub! -->
...
<!-- NOTE: range different from python (half open), in R both numbers are part 
of interval -->
...
* Specific level order can be specified using the `level` argument in `factor()`:
  <pre>
...
* Factors can be converted to _character_ or _numeric_ vectors using 
`as.character()` or `as.numeric()` respectively.
  <pre>
...
* `matrix()` fills matrices by column (default).
...
## Concluding remarks (Subsetting & help)
