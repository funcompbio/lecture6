...
* A new stable version is released every year on April and development versions
  are updated on a daily basis. The last major release (version 4.5.0) took
  place on April, 2025.
...
* You can run R using the [RStudio](https://posit.co/download/rstudio-desktop)
  application.
...
* Using a recent software technology called
  [WebAssembly](https://webassembly.org) (WASM), we can even run R in our
  browser **without having to install anything**. Just try it at
  https://webr.r-wasm.org/latest.

![:scale 70%](img/webr.png)
...
* The most commonly reported way to install package is by giving the
  ...
  * For [Bioconductor](https://bioconductor.org), e.g., package [GSVA](https://bioconductor.org/packages/GSVA):
    <pre>
...
* Vector positions in R are 1-based, this means, **they start at position 1**.
...
values, technically called **levels** in this context.
  <pre>
...
![](img/compbiojob.png)
.footnote[
[Job offer for a computational biologist](https://www.indeed.com/jobs?q=computational+biology+python+R+unix)
]
]
